...
public interface OrderItemsRepository extends JpaRepository<OrderItems, Long> {
	...
	@Query(value = "select i.* from order_items i where i.is_active = 1 and i.menu_id = ?1 and i.order_final_id = ?2", nativeQuery = true)
	OrderItems findByMenuId(Long id,Long fid);
	
	@Query(value = "select i.* from order_items i where i.is_active = 0 and i.menu_id = ?1 and i.order_final_id = ?2", nativeQuery = true)
